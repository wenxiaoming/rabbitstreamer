...
class IntervalArray {
  public:
    ...
    explicit IntervalArray(const IntervalArray &another)
        : m_array(NULL), m_totalsize(another.m_totalsize),
          m_validsize(another.m_validsize) {
        if (m_totalsize) {
            m_array = new BlockInterval[m_totalsize];
            memcpy(m_array, another.m_array,
                   m_validsize * sizeof(BlockInterval));
        }
    };
    ...
    bool operator==(const IntervalArray &a) const {
        if (m_validsize != a.m_validsize)
            return false;
        if (memcmp(m_array, a.m_array, sizeof(BlockInterval) * m_validsize) !=
            0)
            return false;
        return true;
    };
    ...
    void AndOperator(const IntervalArray &another,
                     IntervalArray &result) const {
        BlockInterval temp;
        result.Clear();
        for (uint16_t i = 0; i < another.m_validsize; ++i) {
        ...
        for (uint16_t i = 0; i < m_validsize; ++i) {
            if (m_array[i].start > start) {
                if (m_array[i].start > start + size) {
                    ...
                    m_array[i].size =
                        (m_array[i].start + m_array[i].size) - (start + size);
                ...
                SafeInsert(i + 1, start + size,
                           m_array[i].start + oldSize - (start + size));
                return;
            }
        }
    };
    ...
    void CopyIntervalArray(BlockInterval *targetArray, uint8_t &size) const {
        ...
        if (size >= m_validsize) {
            ...
            std::sort(targetArray, targetArray + size,
                      BlockInterval::cmp_start);
        }
    };

    void Clear() { m_validsize = 0; };

    bool IsEmpty() const { return (m_validsize == 0); }

    uint32_t GetMaxBlockID() const {
        if (m_validsize)
            return m_array[m_validsize - 1].start +
                   m_array[m_validsize - 1].size;
        return 0;
    };
    ...
    uint16_t GetValidSize() const { return m_validsize; };

    uint32_t GetCountInInterval(const uint32_t blockID,
    ...
    void Print() const {
        ...
        for (uint16_t i = 0; i < m_validsize; ++i) {
            printf("START: %d, END: %d, SIZE: %d.\n", m_array[i].start,
                   m_array[i].start + m_array[i].size, m_array[i].size);
        }
    }
  ...
  protected:
    void SafeInsert(uint16_t i, uint32_t start, uint32_t size) {
        ...
        memcpy(m_array + i + 1, temp + i,
               (m_validsize - i) * sizeof(BlockInterval));
    ...
    void SafeDelete(uint16_t i) {
        assert(i < m_validsize);
        ::memmove(m_array + i, m_array + i + 1,
                  (m_validsize - i - 1) * sizeof(BlockInterval));
        --m_validsize;
    }
